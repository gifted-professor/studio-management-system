...
export default function DashboardPage() {
  ...
  const [smartOpportunities, setSmartOpportunities] = useState<
    SmartOpportunity[]
  >([]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // 并行获取所有数据
      const [kpisRes, tasksRes, opportunitiesRes, hotProductsRes] =
        await Promise.all([
      ...
      // 解析数据
      const [kpisData, tasksData, opportunitiesData, hotProductsData] =
        await Promise.all([
          kpisRes.json(),
          tasksRes.json(),
  ...
  const getTaskTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "RETURN_INQUIRY":
        return <Phone className="h-4 w-4 text-chart-3" />;
      default:
            ...
            <div className="space-y-3">
              {smartOpportunities.length === 0 ? (
                ...
                smartOpportunities.map((opportunity) => (
                  <div
                    key={opportunity.id}
                    className="border border-border rounded-lg p-4 hover:border-primary/30 hover:bg-muted/20 transition-all duration-200 group"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
            ...
            <div className="space-y-3">
              {hotProducts.length === 0 ? (
                ...
                hotProducts.map((product, index) => (
                  <div
                    key={index}
                    className="bg-muted/30 rounded-lg p-4 hover:bg-muted/40 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {product.manufacturer} • 平均单价 ¥
                            {product.avgAmount} • 总销售额 ¥
